# 🎮 Auto-Vote-Rating

> Automated voting extension for game server rating sites

[![Version](https://img.shields.io/github/manifest-json/v/JustMordeckai/Auto-Vote-Rating/dev?label=version&color=green)](https://github.com/JustMordeckai/Auto-Vote-Rating/releases)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

## 📋 Overview

Auto-Vote-Rating is a browser extension that automates voting for your favorite game servers across 100+ rating platforms. Never miss a vote again with intelligent scheduling, cooldown detection, and multi-site support.

## ✨ Features

- 🤖 **Automated Voting** - Schedule votes across 100+ rating sites
- ⏰ **Smart Cooldown Detection** - Automatically detects when you can vote again
- 📊 **Statistics Tracking** - Monitor your voting history and success rates
- 🔕 **Silent Mode** - Vote in background without opening tabs
- 🌐 **Multi-Platform Support** - Minecraft, Discord bots, MMO games, and more
- 🔒 **Privacy Focused** - All data stored locally, no external servers

## 🚀 Installation

### For Chrome / Edge / Brave / Opera

1. **Download the extension**
   - Go to [Releases](https://github.com/JustMordeckai/Auto-Vote-Rating/releases)
   - Download the latest `Auto-Vote-Rating-v*.zip` file
   - Extract the ZIP file to a folder on your computer

2. **Load the extension**
   - Open your browser and navigate to:
     - **Chrome**: `chrome://extensions/`
     - **Edge**: `edge://extensions/`
     - **Brave**: `brave://extensions/`
     - **Opera**: `opera://extensions/`
   
3. **Enable Developer Mode**
   - Toggle the "Developer mode" switch (usually in the top right corner)

4. **Load unpacked extension**
   - Click "Load unpacked" button
   - Select the extracted folder
   - The extension should now appear in your browser toolbar

## 📖 Quick Start

1. **Add a Server**
   - Click the extension icon in your browser toolbar
   - Click "Add Server" or "+"
   - Enter the voting page URL
   - Configure voting settings (nickname, intervals, etc.)

2. **Start Auto-Voting**
   - Enable the server in your list
   - The extension will automatically vote at the optimal time
   - Check statistics and logs in the dashboard

## 🎯 Supported Sites (100+)

<details>
<summary>📋 Click to view all supported sites</summary>

### Minecraft Servers (80+)
<a href="https://40servidoresmc.es/">40servidoresmc.es</a>  
<a href="https://atlauncher.com/">atlauncher.com</a>  
<a href="https://best-minecraft-servers.co/">best-minecraft-servers.co</a>  
<a href="https://bestservers.com/">bestservers.com</a>  
<a href="https://craft-list.net/">craft-list.net</a>  
<a href="https://craftbook.cz/">craftbook.cz</a>  
<a href="https://craftlist.org/">craftlist.org</a>  
<a href="https://czech-craft.eu/">czech-craft.eu</a>  
<a href="https://emeraldservers.com/">emeraldservers.com</a>  
<a href="https://findmcserver.com/">findmcserver.com</a>  
<a href="https://ftbservers.com/">ftbservers.com</a>  
<a href="https://gtop100.com/">gtop100.com</a>  
<a href="https://hotmc.ru/">hotmc.ru</a>  
<a href="https://ionmc.top/">ionmc.top</a>  
<a href="https://liste-serveurs-minecraft.org/">liste-serveurs-minecraft.org</a>  
<a href="https://liste-serv-minecraft.fr/">liste-serv-minecraft.fr</a>  
<a href="https://liste-serveurs.fr/">liste-serveurs.fr</a>  
<a href="https://loliland.ru/">loliland.ru</a>  
<a href="https://mclike.com/">mclike.com</a>  
<a href="https://mclist.io/">mclist.io</a>  
<a href="https://mc-liste.de/">mc-liste.de</a>  
<a href="https://mc-lists.org/">mc-lists.org</a>  
<a href="https://mc-monitoring.info/">mc-monitoring.info</a>  
<a href="https://mcrate.su/">mcrate.su</a>  
<a href="https://mc-server-list.com/">mc-server-list.com</a>  
<a href="https://mc-serverlist.cz/">mc-serverlist.cz</a>  
<a href="https://mcserver-list.eu/">mcserver-list.eu</a>  
<a href="https://mc-servers.com/">mc-servers.com</a>  
<a href="https://mcservers.top/">mcservers.top</a>  
<a href="https://mcservertime.com/">mcservertime.com</a>  
<a href="https://mcservidores.com/">mcservidores.com</a>  
<a href="https://mctop.su/">mctop.su</a>  
<a href="https://minebrowse.com/">minebrowse.com</a>  
<a href="https://minecraftbestservers.com/">minecraftbestservers.com</a>  
<a href="https://minecraft.buzz/">minecraft.buzz</a>  
<a href="https://minecraft.global/">minecraft.global</a>  
<a href="https://minecraft-index.com/">minecraft-index.com</a>  
<a href="https://minecraftiplist.com/">minecraftiplist.com</a>  
<a href="https://minecraftkrant.nl/">minecraftkrant.nl</a>  
<a href="https://minecraft-list.cz/">minecraft-list.cz</a>  
<a href="https://minecraftlist.org/">minecraftlist.org</a>  
<a href="https://minecraft.menu/">minecraft.menu</a>  
<a href="https://minecraftrating.ru/">minecraftrating.ru</a>  
<a href="https://minecraft-server.eu/">minecraft-server.eu</a>  
<a href="https://minecraft-server.net/">minecraft-server.net</a>  
<a href="https://minecraft-server.sk/">minecraft-server.sk</a>  
<a href="https://minecraft-serverlist.com/">minecraft-serverlist.com</a>  
<a href="https://minecraft-server-list.com/">minecraft-server-list.com</a>  
<a href="https://minecraft-serverlist.net/">minecraft-serverlist.net</a>  
<a href="https://minecraftserver.sk/">minecraftserver.sk</a>  
<a href="https://minecraft-servers.biz/">minecraft-servers.biz</a>  
<a href="https://minecraft-servers.de/">minecraft-servers.de</a>  
<a href="https://minecraft-servers-list.org/">minecraft-servers-list.org</a>  
<a href="https://minecraftservers.biz/">minecraftservers.biz</a>  
<a href="https://minecraftservers.org/">minecraftservers.org</a>  
<a href="https://minecraftservers100.com/">minecraftservers100.com</a>  
<a href="https://minecraftservery.eu/">minecraftservery.eu</a>  
<a href="https://minecraftsurvivalservers.com/">minecraftsurvivalservers.com</a>  
<a href="https://minecraftsurvivalservers.net/">minecraftsurvivalservers.net</a>  
<a href="https://minehut.com/">minehut.com</a>  
<a href="https://minelist.net/">minelist.net</a>  
<a href="https://mineserv.top/">mineserv.top</a>  
<a href="https://mineservers.com/">mineservers.com</a>  
<a href="https://minestatus.net/">minestatus.net</a>  
<a href="https://misterlauncher.org/">misterlauncher.org</a>  
<a href="https://monitoringminecraft.ru/">monitoringminecraft.ru</a>  
<a href="https://pixelmon-server-list.com/">pixelmon-server-list.com</a>  
<a href="https://pixelmonservers.com/">pixelmonservers.com</a>  
<a href="https://planetminecraft.com/">planetminecraft.com</a>  
<a href="https://play-minecraft-servers.com/">play-minecraft-servers.com</a>  
<a href="https://rpg-paradize.com/">rpg-paradize.com</a>  
<a href="https://serverlist.games/">serverlist.games</a>  
<a href="https://serverlist101.com/">serverlist101.com</a>  
<a href="https://serverliste.net/">serverliste.net</a>  
<a href="https://serverlocator.com/">serverlocator.com</a>  
<a href="https://serverpact.com/">serverpact.com</a>  
<a href="https://servers-minecraft.net/">servers-minecraft.net</a>  
<a href="https://serveur-minecraft.com/">serveur-minecraft.com</a>  
<a href="https://serveur-minecraft.fr/">serveur-minecraft.fr</a>  
<a href="https://serveur-minecraft-vote.fr/">serveur-minecraft-vote.fr</a>  
<a href="https://serveur-prive.net/">serveur-prive.net</a>  
<a href="https://serveurliste.com/">serveurliste.com</a>  
<a href="https://serveurs-mc.net/">serveurs-mc.net</a>  
<a href="https://serveursminecraft.org/">serveursminecraft.org</a>  
<a href="https://servidoresdeminecraft.es/">servidoresdeminecraft.es</a>  
<a href="https://technicservers.com/">technicservers.com</a>  
<a href="https://tekkitserverlist.com/">tekkitserverlist.com</a>  
<a href="https://tmonitoring.com/">tmonitoring.com</a>  
<a href="https://top100arena.com/">top100arena.com</a>  
<a href="https://topcraft.club/">topcraft.club</a>  
<a href="https://topg.org/">topg.org</a>  
<a href="https://top-games.net/">top-games.net / top-serveurs.net</a>  
<a href="https://topmcservers.com/">topmcservers.com</a>  
<a href="https://top-mc-servers.net/">top-mc-servers.net</a>  
<a href="https://topminecraft.io/">topminecraft.io</a>  
<a href="https://topminecraftservers.org/">topminecraftservers.org</a>  
<a href="https://topservers.com/">topservers.com</a>  
<a href="https://trackyserver.com/">trackyserver.com</a>  
<a href="https://trackingservers.cloud/">trackingservers.cloud</a>  
<a href="https://xtremetop100.com/">xtremetop100.com</a>

### Discord Bots
<a href="https://botlist.me/">botlist.me</a>  
<a href="https://discadia.com/">discadia.com</a>  
<a href="https://discord.boats/">discord.boats</a>  
<a href="https://discordbotlist.com/">discordbotlist.com</a>  
<a href="https://discords.com/">discords.com</a>  
<a href="https://infinitybots.gg/">infinitybots.gg</a>  
<a href="https://rovelstars.com/">rovelstars.com</a>  
<a href="https://top.gg/">top.gg</a>

### MMO & Other Games
<a href="https://7daystodie-servers.com/">7daystodie-servers.com</a>  
<a href="https://ark-servers.net/">ark-servers.net</a>  
<a href="https://arma3-servers.net/">arma3-servers.net</a>  
<a href="https://atlas-servers.io/">atlas-servers.io</a>  
<a href="https://conan-exiles.com/">conan-exiles.com</a>  
<a href="https://counter-strike-servers.net/">counter-strike-servers.net</a>  
<a href="https://cubeworld-servers.com/">cubeworld-servers.com</a>  
<a href="https://curseforge.com/">curseforge.com</a>  
<a href="https://dayz-servers.org/">dayz-servers.org</a>  
<a href="https://ecoservers.io/">ecoservers.io</a>  
<a href="https://empyrion-servers.com/">empyrion-servers.com</a>  
<a href="https://genshindrop.com/">genshindrop.com</a>  
<a href="https://gmod-servers.com/">gmod-servers.com</a>  
<a href="https://hoyolab.com/">hoyolab.com</a>  
<a href="https://hurtworld-servers.net/">hurtworld-servers.net</a>  
<a href="https://hytale-servers.io/">hytale-servers.io</a>  
<a href="https://life-is-feudal.org/">life-is-feudal.org</a>  
<a href="https://minecraftpocket-servers.com/">minecraftpocket-servers.com</a>  
<a href="https://minecraft-tracker.com/">minecraft-tracker.com</a>  
<a href="https://miscreated-servers.com/">miscreated-servers.com</a>  
<a href="https://mmorpg.top/">mmorpg.top</a>  
<a href="https://mmotop.ru/">mmotop.ru</a>  
<a href="https://mmovote.ru/">mmovote.ru</a>  
<a href="https://reign-of-kings.net/">reign-of-kings.net</a>  
<a href="https://rust-servers.net/">rust-servers.net</a>  
<a href="https://space-engineers.com/">space-engineers.com</a>  
<a href="https://squad-servers.com/">squad-servers.com</a>  
<a href="https://starbound-servers.net/">starbound-servers.net</a>  
<a href="https://teamspeak-servers.org/">teamspeak-servers.org</a>  
<a href="https://terraria-servers.com/">terraria-servers.com</a>  
<a href="https://tf2-servers.com/">tf2-servers.com</a>  
<a href="https://top-mmogames.ru/">top-mmogames.ru</a>  
<a href="https://unturned-servers.net/">unturned-servers.net</a>  
<a href="https://warface.com/">warface.com</a>  
<a href="https://wargm.ru/">wargm.ru</a>  
<a href="https://wurm-unlimited.com/">wurm-unlimited.com</a>

</details>

## ⚙️ Configuration

### Basic Settings
- **Nickname**: Set your in-game username for voting
- **Silent Vote**: Vote in background without opening tabs
- **Timeout**: Page load timeout (default: 10s)
- **Vote Timeout**: Maximum time to wait for vote completion (default: 15min)

### Advanced Options
- **Custom Vote URL**: Add support for unlisted rating sites
- **Randomize Votes**: Add random delay between votes (anti-bot detection)
- **Debug Mode**: Enable detailed console logging
- **Priority Voting**: Vote on high-priority servers first

### Notifications
Customize when to receive notifications:
- ✅ Success notifications
- ⚠️ Warning notifications
- ❌ Error notifications
- ℹ️ Info notifications

## 📊 Statistics

Track your voting activity:
- **Total Votes**: Lifetime successful votes
- **Monthly Votes**: Current month statistics
- **Today's Votes**: Daily progress
- **Success Rate**: Vote success percentage
- **Error Count**: Failed vote attempts
- **Last Vote**: Timestamp of most recent vote

## 🔧 Technical Details

### Built With
- **Manifest V3**: Latest Chrome extension standard
- **IndexedDB**: Local data storage
- **Modern JavaScript**: ES6+ features

### Browser Compatibility
- ✅ Chrome 105+
- ✅ Edge 105+
- ✅ Brave 1.45+
- ✅ Opera 91+
- ✅ All Chromium-based browsers

### Key Features
- Smart cooldown detection (new HTML structures)
- Captcha detection and handling
- Network error recovery
- Automatic retry on failure

## 🐛 Troubleshooting

### Extension not working?
1. Make sure Developer Mode is enabled
2. Try reloading the extension (`chrome://extensions/` > Reload button)
3. Check browser console for errors (F12)

### Votes not being registered?
1. Verify the voting URL is correct
2. Check if the site requires manual captcha solving
3. Enable debug mode to see detailed logs

### Other issues?
- [Report a bug](https://github.com/JustMordeckai/Auto-Vote-Rating/issues/new)
- [Check existing issues](https://github.com/JustMordeckai/Auto-Vote-Rating/issues)

## 🤝 Contributing

Contributions are welcome! 

1. Fork the repository
2. Create a feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

## 📄 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

## � Credits

### Original Developers
This project was originally created and maintained by:
- **[Serega007](https://github.com/Serega007RU)** - Original creator and primary developer
- All the original contributors who built the foundation of this extension

### Current Maintainer
- **[JustMordeckai](https://github.com/JustMordeckai)** - Current maintainer and developer (v1.0.0+)

This is a continuation and modernization of the original Auto-Vote-Rating project. Special thanks to Serega007 and all original developers for their incredible work and for creating such a useful tool for the gaming community.

### Libraries Used
- **[IDB](https://github.com/jakearchibald/idb)** - IndexedDB wrapper
- **[LinkeDOM](https://github.com/WebReflection/linkedom)** - Lightweight DOM implementation ([Polyfill](https://github.com/regseb/castkodi/tree/main/src/polyfill))
- **[HackTimer](https://github.com/turuslan/HackTimer)** - Accurate timers for background workers

## 💬 Support

- 🐛 [Report a Bug](https://github.com/JustMordeckai/Auto-Vote-Rating/issues/new)
- 💡 [Request a Feature](https://github.com/JustMordeckai/Auto-Vote-Rating/issues/new)
- ❓ [Ask a Question](https://github.com/JustMordeckai/Auto-Vote-Rating/discussions)

## ⭐ Show Your Support

Give a ⭐️ if this project helped you!

---

<div align="center">
Made with ❤️ by <a href="https://github.com/JustMordeckai">JustMordeckai</a><br>
Based on the original work by <a href="https://github.com/Serega007RU">Serega007</a>
</div>
